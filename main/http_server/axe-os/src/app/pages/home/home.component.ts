...
export class HomeComponent implements AfterViewChecked, OnInit, OnDestroy {
  ...
  public get dataFanSpeed(): number[] { return this.chartState.fanSpeed; }
  public set dataFanSpeed(v: number[]) { this.chartState.fanSpeed = v; }
  ...
  constructor(
    ...
    const textColor = getComputedStyle(document.documentElement).getPropertyValue('--text-basic-color').trim();
    const textColorSecondary = getComputedStyle(document.documentElement).getPropertyValue('--text-hint-color').trim();

    this.heatChartData = {
      labels: this.chartState.labels,
      datasets: createHeatDatasets({ t: (k) => this.translateService.instant(k), series: this.chartState as unknown as HomeChartSeriesRefs }),
    };
    ...
    const cfg = createHomeChartConfig({
      series: {
        labels: this.chartState.labels,
        hr1m: this.chartState.hr1m,
        hr10m: this.chartState.hr10m,
        hr1h: this.chartState.hr1h,
        hr1d: this.chartState.hr1d,
        vregTemp: this.chartState.vregTemp,
        asicTemp: this.chartState.asicTemp,
        fanSpeed: this.chartState.fanSpeed,
      } as HomeChartSeriesRefs,
...
ngOnInit() {
  ...
  public updateTimeFormat(): void {
    ...
    if (this.chartOptions.scales?.x?.time?.displayFormats) {
      ...
      if (this.heatChart) {
        const heatX = (this.heatChart.options.scales as any)?.x;
        if (heatX?.time?.displayFormats) {
          heatX.time.displayFormats.hour = timeFormat;
        }
        (this.heatChart.options.plugins as any).tooltip.callbacks.title = (context: any) => {
          const date = new Date(context[0].parsed.x);
          const format = this.localStorage.getItem('timeFormat') === '12h';
          return format
            ? date.toLocaleString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true, month: 'short', day: 'numeric' })
...
private setAxisPadding(cfg: any, persist: boolean = false): void {
  ...
  private updateAxesScaleAdaptive(): void {
    ...
    const series = {
      ...
      fanSpeed: this.dataFanSpeed,
    } as HomeChartSeriesRefs;

    const { bounds } = computeHomeChartScales({
      labels,
      xMinMs,
      xMaxMs,
      series,
  ...
  private sanitizeFanSpeed(v: any): number {
    const n = Number(v);
    if (!Number.isFinite(n)) return NaN;
    if (n < HOME_CFG.sanitize.fanSpeedMin) return NaN;
    if (n > HOME_CFG.sanitize.fanSpeedMax) return NaN;
    return n;
  }
  ...
  private updateChartData(data: any): void {
    ...
    const fanSpeed: number[] = Array.isArray(data.fanSpeed) ? data.fanSpeed : [];

    const n = Math.min(timestamps.length, h1m.length, h10m.length, h1h.length, h1d.length, vreg.length, asic.length, fanSpeed.length);
    if (n <= 0) return;
    ...
    for (let i = 0; i < n; i++) {
      ...
      newData.push({
        ...
        fanSpeed: Number(fanSpeed[i]) / 100.0,
      });
    }
    ...
    for (const entry of newData) {
      ...
      const fanSpeedEnabled = this.warmupMachine.isFanSpeedEnabled();
      ...
      const fanSpeedRaw = this.sanitizeFanSpeed(entry.fanSpeed);
      ...
      const applyFanSpeed = (key: string, v: number, thr: number): number => {
        if (!Number.isFinite(v)) return NaN;
        return this.graphGuardEngine.apply(key, v, thr);
      };
      ...
      if (lastIdx >= 0 && this.dataLabel[lastIdx] === entry.timestamp) {
        ...
        this.dataFanSpeed[lastIdx] = fanSpeedEnabled ? applyFanSpeed('fanSpeed', fanSpeedRaw, HOME_CFG.graphGuard.thresholds.fanSpeed) : NaN;
      ...
      this.dataFanSpeed.push(fanSpeedEnabled ? applyFanSpeed('fanSpeed', fanSpeedRaw, HOME_CFG.graphGuard.thresholds.fanSpeed) : NaN);
  ...
  private updateChart() {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      this.chart!.update();
      if (this.heatChart) {
        this.heatChart.data.labels = this.dataLabel;
